"""
SD1.5 Inpainting + SFT inference script for RLBench evaluation.
Generates inpainted images using the SFT fine-tuned model.
"""
import torch
import os
...
def parse_args():
    parser = argparse.ArgumentParser(description="SD1.5 Inpainting + SFT inference")
    parser.add_argument('--model_path', type=str, default="",
                        help="Path to base SD1.5 Inpainting model")
    parser.add_argument('--unet_path', type=str, default="",
                        help="Path to SFT fine-tuned UNet weights (.safetensors)")
    parser.add_argument('--image_save_path', type=str, default="./results/sd15_sft",
...
def main():
    ...
    # Load SFT fine-tuned UNet weights
    if args.unet_path and os.path.exists(args.unet_path):
        print(f"Loading SFT fine-tuned UNet from {args.unet_path}")
        unet_state_dict = load_file(args.unet_path)
        pipe.unet.load_state_dict(unet_state_dict)
        print("Successfully loaded SFT fine-tuned UNet")
